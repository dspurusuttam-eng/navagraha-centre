...
const landing = requireInsightsSeoLanding("astrology-guides");
...
export default async function InsightsAstrologyGuidesPage() {
  const entries = await getContentAdapter().listPublishedEntries();

  return (
    <InsightsSeoLandingPage
      landing={landing}
      entries={getEntriesForInsightsSeoLanding("astrology-guides", entries)}
    />
  );
}
