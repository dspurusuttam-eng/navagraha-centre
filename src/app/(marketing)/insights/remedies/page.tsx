...
const category = requireInsightsCategory("remedies");
...
export default async function InsightsRemediesCategoryPage() {
  const entries = await getContentAdapter().listPublishedEntries();

  return (
    <InsightsCategoryPage
      category={category}
      entries={getEntriesForInsightsCategory("remedies", entries)}
      relatedCategories={selectRelatedCategoryConfigs(insightsCategories, "remedies")}
    />
  );
}
