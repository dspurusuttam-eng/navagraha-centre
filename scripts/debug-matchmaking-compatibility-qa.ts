...
import { buildMatchmakingCompatibilityFoundation } from "@/modules/astrology/matchmaking";
...
const ready = buildMatchmakingCompatibilityFoundation({
  personA: {
    chart: chartA,
...
assert.equal(ready.data?.manglikAnalysis.personA.status, "ready");
assert.equal(ready.data?.manglikAnalysis.personB.status, "ready");
assert.equal(ready.data?.manglikAnalysis.personA.lagnaCheck.status, "ready");
assert.equal(ready.data?.manglikAnalysis.personA.moonCheck.status, "ready");
assert.equal(ready.data?.manglikAnalysis.personA.venusCheck.status, "ready");
assert.equal(typeof ready.data?.compatibilityInsights.emotionalCompatibility, "string");
assert.equal(typeof ready.data?.compatibilityInsights.communicationCompatibility, "string");

const unavailable = buildMatchmakingCompatibilityFoundation({
  personA: {
    chart: chartA,
    savedKundliId: "saved-kundli-a",
    label: "Person A",
  ...
  JSON.stringify(
    {
      ready: {
        ...
        manglikAnalysis: ready.data?.manglikAnalysis ?? null,
        compatibilityInsights: ready.data?.compatibilityInsights ?? null,
      },
      unavailable: {
