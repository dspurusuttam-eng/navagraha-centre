...
const category = requireInsightsCategory("career");
...
export default async function InsightsCareerCategoryPage() {
  const entries = await getContentAdapter().listPublishedEntries();

  return (
    <InsightsCategoryPage
      category={category}
      entries={getEntriesForInsightsCategory("career", entries)}
      relatedCategories={selectRelatedCategoryConfigs(insightsCategories, "career")}
    />
  );
}
