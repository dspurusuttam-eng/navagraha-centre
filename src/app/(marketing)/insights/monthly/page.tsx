...
const category = requireInsightsCategory("monthly");
...
export default async function InsightsMonthlyCategoryPage() {
  const entries = await getContentAdapter().listPublishedEntries();

  return (
    <InsightsCategoryPage
      category={category}
      entries={getEntriesForInsightsCategory("monthly", entries)}
      relatedCategories={selectRelatedCategoryConfigs(insightsCategories, "monthly")}
    />
  );
}
